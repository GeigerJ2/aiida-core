...
class ShellJob(CalcJob):
    ...
    def serialize_parser(cls, value: t.Any) -> CallableData:
        ...
        :returns: The :class:`~aiida.orm.nodes.data.callable.CallableData` node recording the callable.
    ...
    def validate_parser(cls, value: t.Any, _: t.Any) -> str | None:
        ...
        # The hook is called positionally, so every parameter has to be suppliable that way, in this order: one that
        # can only be passed by keyword is never supplied, and a reversed pair binds the wrong way round.
        supplied_positionally = value.parameters == value.positional_parameters
        shape_is_supported = value.positional_parameters in (['dirpath'], ['dirpath', 'parser'])

        if not (supplied_positionally and shape_is_supported):
            correct_signature = (
