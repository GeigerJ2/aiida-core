    ...
    * one that cannot, a lambda or a closure, is recorded by a fingerprint of its serialized form.
    ...
    class AttributesModel(Data.AttributesModel):
        ...
        importable: bool = OrmMetadataField(
            description='Whether the callable can be recovered by importing it under its recorded name', read_only=True
        )
        ...
        source_file: str | None = OrmMetadataField(
            None, description='The file the callable was defined in', read_only=True
        )
        source_line: int | None = OrmMetadataField(
            None, description='The line in `source_file` at which the definition starts', read_only=True
        )
    ...
    def __init__(self, value: t.Callable[..., t.Any], *, entry_point: str | None = None, **kwargs: t.Any) -> None:
        ...
        distribution, version = distributions.distribution_of(module) if importable and module else (None, None)
        commit = distributions.commit_of(distribution) if distribution is not None else None
        ...
        attributes: dict[str, t.Any] = {
            ...
            self.KEY_ATTRIBUTES_DISTRIBUTION: distribution,
            self.KEY_ATTRIBUTES_VERSION: version,
            self.KEY_ATTRIBUTES_COMMIT: commit,
    ...
    def positional_parameters(self) -> list[str] | None:
        ...
        A caller that invokes the callable positionally has to check these rather than :meth:`parameters`, which
        includes those that can only be passed by keyword.
        """
        recorded = self._recorded_parameters
