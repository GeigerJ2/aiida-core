...
def _find_origin(module_name: str, search_paths: tuple[str, ...]) -> str | None:
    ...
    it is asked about are those of a running daemon, which do not change while that daemon runs. The cache holds
    more entries than an interpreter has modules, since :func:`modules_missing_from` asks about all of them in one
