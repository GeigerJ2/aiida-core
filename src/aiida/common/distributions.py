...
__all__ = ('commit_of', 'distribution_of')
...
@functools.cache
def _editable_roots() -> tuple[tuple[str, str, str], ...]:
    """Return ``(distribution, version, directory)`` for every directory an editable install adds to ``sys.path``.
    ...
    for distribution in importlib_metadata.distributions():
        ...
        for entry in distribution.files or []:
            ...
            for line in content.splitlines():
                ...
                roots.append((name, version, str(pathlib.Path(stripped).resolve())))

    return tuple(roots)


def _editable_distribution(top_level: str) -> tuple[str | None, str | None]:
    ...
    :returns: The name of the distribution and its version, or ``(None, None)`` if none accounts for it.
    """
    ...
    for name, version, root in _editable_roots():
        if root == directory:
            return name, version
...
def distribution_of(module: str) -> tuple[str | None, str | None]:
    """Return the distribution that provides ``module`` and its version, as far as they can be determined.
    ...
    the package is still here rather than when something needs it, by which time it may well be gone.

    :param module: The dotted name of the module to account for.
    :returns: The name of the distribution and its version, either of which is ``None`` if it is not known.
    """
...
def commit_of(distribution: str) -> str | None:
