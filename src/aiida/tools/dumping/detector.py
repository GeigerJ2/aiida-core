...
class DumpChangeDetector:
    ...
    def detect_new_nodes(self, group: orm.Group | None = None, filters: dict | None = None) -> DumpNodeStore:
        if group:
    ...
    def _filter_diff_for_group(self, diff_result: Dict, group_uuid: str) -> Dict:
        ...
        filtered_diff = {
            'deleted_groups': [g for g in diff_result['deleted_groups'] if g.get('uuid') == group_uuid],
            'modified_groups': [g for g in diff_result['modified_groups'] if g.get('uuid') == group_uuid],
            'new_groups': [g for g in diff_result['new_groups'] if g.get('uuid') == group_uuid],
        ...
        for node_uuid, changes in diff_result['nodes_membership_changes'].items():
            ...
            removed_from = [g for g in changes.get('removed_from', []) if g == group_uuid]
    ...
    filters: dict | None = None,
        ...
        if effective_scope == NodeDumpGroupScope.IN_GROUP and group is None:
            ...
            logger.debug('Scope is IN_GROUP but no group provided. Using ANY scope instead.')
