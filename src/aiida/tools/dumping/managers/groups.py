...
class GroupManager:
    ...
    def __init__(self, config, dump_paths, dump_logger, node_processor=None):
        self.config = config
    ...
    def handle_group_dump(self, group, changes):
        ...
        if self.node_processor:
            self.node_processor.dump_nodes(changes['new_nodes'], group)
        else:
    ...
    def update_group(self, group_info):
        ...
        try:
            ...
            if 'nodes_added' in group_info and self.node_processor:
                ...
                for node_uuid in group_info['nodes_added']:
                    try:
                        ...
                        # Dump the node
                        self.node_processor.dump_process(node, group)
