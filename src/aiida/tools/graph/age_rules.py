...
class RuleSequence(Operation):
    ...
    def __init__(
        ...
        *,
        iteration_callback: Callable[[int, Basket], None] | None = None,
    ):
        ...
        :param iteration_callback: called after each iteration with the number of iterations done so
            far and the basket of all entities visited so far, e.g. to report progress of long
            traversals. The basket is the live internal accumulator and must be treated as read-only;
            mutating or retaining it corrupts the traversal state.
        """
        ...
        for rule in rules:
            if not isinstance(rule, Operation):
                # Bare raise: mypy proves this branch dead from the annotation and exempts only
                # lone-`raise` blocks from `warn_unreachable`, so `msg = ...` would fail here.
    ...
    def run(self, operational_set: Basket) -> Basket:
        ...
        while operational_set and self._iterations_done < self._max_iterations:
            ...
            if self._iteration_callback is not None:
                self._iteration_callback(self._iterations_done, self._visits_set)
