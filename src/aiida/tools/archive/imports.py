...
def _import_authinfos(
    ...
    # retrieve existing user_id / computer_id - batch the queries to avoid filter_size limits
...
def _merge_node_extras(
    ...
    uuid_list = list(backend_uuid_id.keys())
    ...
    if mode == ('n', 'c', 'u'):
        ...
        with get_progress_reporter()(desc='Replacing extras', total=num_existing) as progress:
            for _, uuid_batch in batch_iter(uuid_list, filter_size):
                input_extras = (
                    QueryBuilder(backend=backend_from)
                    .append(orm.Node, tag='node', filters={'uuid': {'in': uuid_batch}}, project=['uuid', 'extras'])
                    .order_by([{'node': 'uuid'}])
                )
    ...
    with get_progress_reporter()(desc='Merging extras', total=num_existing) as progress:
        for _, uuid_batch in batch_iter(uuid_list, filter_size):
            input_extras = (
                QueryBuilder(backend=backend_from)
                .append(orm.Node, tag='node', filters={'uuid': {'in': uuid_batch}}, project=['uuid', 'extras'])
            ...
            if not input_extras.count() == backend_extras.count():
                raise ImportValidationError(
                    f'Number of Nodes in archive batch ({input_extras.count()}) and backend batch ({backend_extras.count()}) do not match'
                )
...
def _make_import_group(
    ...
    # Add all the nodes to the Group - batch to avoid memory issues with large node sets
    node_ids_to_add = [node_id for node_id in node_ids_archive_backend.values() if node_id not in group_node_ids]

    with get_progress_reporter()(desc='Adding all Node(s) to the import Group', total=len(node_ids_to_add)) as progress:
        for nrows, node_batch in batch_iter(node_ids_to_add, batch_size):
            rows = [{'dbgroup_id': group_id, 'dbnode_id': node_id} for node_id in node_batch]
            backend_to.bulk_insert(EntityTypes.GROUP_NODE, rows)
            progress.update(nrows)
