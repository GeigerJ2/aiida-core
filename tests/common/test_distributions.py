...
def test_distribution_of_an_installed_package():
    ...
    assert distributions.distribution_of('yaml') == ('PyYAML', importlib_metadata.version('PyYAML'))
...
def test_distribution_of_the_standard_library():
    ...
    assert distributions.distribution_of('json') == (None, None)
...
def test_distribution_reported_more_than_once(monkeypatch):
    ...
    assert distributions.distribution_of('demo.sub') == ('demo-editable', '0.1.0')
...
def test_distribution_of_an_editable_install(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(distributions, '_editable_roots', lambda: (('demo-editable', '0.1.0', str(tmp_path)),))
...
def test_commit_of_a_distribution_installed_from_a_repository(monkeypatch):
    ...
    assert distributions.commit_of('demo') == 'a' * 40
...
def test_commit_of_a_distribution_installed_from_a_release(monkeypatch):
    ...
    assert distributions.commit_of('demo') is None
...
def test_commit_of_a_distribution_that_is_not_installed():
    ...
    assert distributions.commit_of('no-such-distribution') is None
