...
The graph itself shows the wiring: a single `ShellJob<gsrd@localhost>` task with the failing input file pre-bound. The handler is a piece of metadata attached to that task, not a node in the graph &mdash; you can see its presence via `tasks.ShellJob.error_handlers`.
...
The ShellJob now succeeds &mdash; on its second attempt, with the handler-corrected `dt`.
...
:::{dropdown} When to write an error handler &mdash; and when not to
:icon: info
...
- **Domain-meaningful exit codes** through `spec.exit_code(...)` &mdash; the same mechanism the error-handler section just hooked into.
...
After `pip install`, the new plugin is discoverable to AiiDA: anyone can `load_node`, query, share archives produced by it, and write WorkChains/WorkGraphs that use it &mdash; just as you used `aiida_shell.ShellJob` throughout this tutorial.
...
:::{dropdown} How to choose &mdash; `aiida-shell` vs a CalcJob plugin
:icon: info
...
The same workflow written as a WorkGraph would be a short `@task.graph()` body wiring three calls together &mdash; structurally closer to what you wrote in Module 3.
...
:::{dropdown} Caveats &mdash; what gets cached
:icon: info
...
{ref}`topics:provenance:caching:control-hashing` is the page to read when that happens &mdash; it covers `_aiida_hash`, `_hash_ignored_inputs`, and how to invalidate stale hashes.
:::
...
Project-scoped AiiDA environments &mdash; one Python venv plus profile per investigation, switchable with a single command. Solves the "ten ongoing studies, ten conflicting dependencies" problem.
...
The "wrap any executable, no plugin code required" path you have been using since {ref}`Module 1 <tutorial:module1>`. The fastest way to bring a new code into AiiDA &mdash; promote it to a CalcJob plugin once the interface stabilises.
...
Helper classes for orchestrating very large submission queues &mdash; "run this WorkChain over 100,000 input structures, throttled to N concurrent jobs". The standard partner for high-throughput screening.
...
Jupyter-based front-end on top of AiiDA. Hosts the [Quantum ESPRESSO app](https://aiidalab-qe.readthedocs.io) and friends &mdash; turnkey GUIs for full simulation protocols, talking to your local AiiDA profile.
...
- **Browse the plugin registry** for adjacent tools (workflow controllers, queue managers, GUI front-ends) before you write something yourself &mdash; the answer is often "someone already did this".
...
- **Contribute back.** AiiDA is community-maintained. Improvements to docs, tutorials, plugins, and the core engine itself all start from the same place &mdash; the [CONTRIBUTING.md guide](https://github.com/aiidateam/aiida-core/blob/main/CONTRIBUTING.md) in the aiida-core repository.
